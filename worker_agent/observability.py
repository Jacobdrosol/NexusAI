...
def install_observability(app: FastAPI) -> None:
    ...
    metrics.register_counter(
        ...
        "Total HTTP requests handled by worker_agent",
        ["method", "path", "status"],
    )
    metrics.register_counter(
        "nexus_worker_agent_http_errors_total",
        "Total HTTP 5xx responses emitted by worker_agent",
        ["method", "path", "status"],
    )
    metrics.register_histogram(
        "nexus_worker_agent_http_request_duration_seconds",
        "worker_agent request latency in seconds",
        ["method", "path"],
    ...
    async def metrics_endpoint(request: Request) -> Any:
        ...
        metrics.set_gauge("nexus_worker_agent_inference_inflight", {}, inflight)
