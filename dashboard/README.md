...
| `launchable_bots(bots, *, surface, blocked_bot_ids=None)` | Filters a list of enabled bot dicts to only those with an enabled launch profile visible on the requested surface (`"overview"` or `"tasks"`), optionally excluding bots with known blocked tooling readiness. Returns sorted list. |
