...
Conversation create and route-default endpoints validate default bot readiness and direct-chat eligibility before proxying changes to the control plane. Conversation create also validates scope and blocks workspace-tool flags for unscoped chats. Message and stream send endpoints validate attachment shape, file count, and declared byte total before proxying. Direct message responses, message-list responses, and chat stream `user_message`/`assistant_message` SSE events normalize message metadata before returning it to the browser, including stripping unsafe attachment preview data URLs while preserving raster image previews. The chat composer capability summary and image-attachment preflight use the selected bot's configured backend model. The effective-context API exposes that model route, image-attachment support, memory gates, workspace-tool gates, and inline-coding gates as JSON for future mobile clients, automation monitors, and pre-send browser checks. The browser composer calls that endpoint before dispatch when workspace tools or inline coding are requested, reducing stale local policy decisions before the final server enforcement.

Direct conversation eligibility is controlled by `routing_rules.direct_chat.enabled`, independently of the bot's worker enablement, profile, and tool access. It is enforced by the dashboard create and route-default endpoints as well as the web selector. Existing bots without that setting retain their prior heuristic eligibility until saved; newly configured bots use the explicit gate. Conversation-level model overrides are retired: the selected bot's configured backend model is authoritative and saving a route clears any legacy override.
