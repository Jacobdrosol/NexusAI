...
- Verification: dashboard auth/mobile-contract tests pass, including manifest-backed release metadata. The signed release build has completed previously with the persistent signing key. Server publication reached the Android SDK container and persistent signing-key mount; the wrapper is invoked through `sh` so publishing does not depend on its executable bit. The current workstation shell does not have `ANDROID_HOME` or Docker Desktop available, so the final containerized deployment build is verified on the server.
